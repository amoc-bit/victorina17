...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        bot = context.bot
        logger.info(f"Запущен subscription_manager ")
        await subscription_manager.initialize(bot)
        logger.info(f"Инициализация SubscriptionManager")

        is_subscribed = await subscription_manager.check_subscription(
            context.bot, update.effective_user.id  # передаем bot как параметр
        )
        if not is_subscribed:
            await subscription_manager.send_subscription_request(
                update.effective_chat.id, context.bot  # передаем bot как параметр
            )
        ...
        user, created = await User.objects.aget_or_create(
            ...
            defaults={"username": username, "last_activity": timezone.now()},
        )
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            REGISTRATION_COMPLETE.format(name=name), reply_markup=ReplyKeyboardRemove()
        )
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отмена операции"""
    await update.message.reply_text(
        OPERATION_CANCELLED, reply_markup=ReplyKeyboardRemove()
    )
    return ConversationHandler.END
    ...
    try:
        ...
        async for admin in admins:
            try:
                ...
                logger.error(
                    f"Error sending notification to admin {admin.telegram_id}: {e}"
                )
    except Exception as e:
